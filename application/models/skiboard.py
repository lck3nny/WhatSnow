...
class SkiBoard():
    ...
    def nameify(strings):
        for s in strings:
            try:
                s = SkiBoard.normalise_brand_model(s)
            except:
                continue
    ...
    def is_duplicate(category, brand, model, year):
        ...
        slug = SkiBoard.slugify([brand, model, str(year)])
    ...
    def create(brand, model, year, category, author=None):
        ...
        try:
            ...
            create_time, skiboard = db.collection('SkiBoards').add({
                ...
                'author': author,
                'slug': SkiBoard.slugify([brand, model, str(year)]),
                'name': SkiBoard.nameify([brand, model, str(year)])
            })
    ...
    def update_info(id, update_params={}, size_params={}):
        ...
        if skiboard.exists:
            ...
            resp = SkiBoard.update_es(id, skiboard)
    ...
    def search(query, es_index='skiboards'):
        ...
        # Connect to ElasticSearch
        es_client = Elasticsearch([es_config['alturl']], basic_auth=(es_config['altkey'], es_config['altsecret']))
        print(es_client.ping())
        idx_manager = IndicesClient(es_client)
        #print(es_index)
        #print(idx_manager.get(index=es_index))

        active_index = list(idx_manager.get(index=es_index).keys())[0]
    ...
    def format_params(unformatted, units):
        ...
        for key in unformatted:
            matched, confidence = SkiBoard.match_param(key)
